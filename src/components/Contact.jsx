import { motion } from 'framer-motion'

const EmailIcon = () => (
  <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25H4.5a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5H4.5a2.25 2.25 0 00-2.25 2.25m19.5 0-9.75 6.75L2.25 6.75" />
  </svg>
)

const PhoneIcon = () => (
  <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
  </svg>
)

const GitHubIcon = () => (
  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z" />
  </svg>
)

const LinkedInIcon = () => (
  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
  </svg>
)

const LINK_ICONS = {
  GitHub: <GitHubIcon />,
  LinkedIn: <LinkedInIcon />,
}

const Contact = ({ content }) => {
  return (
    <section id="contact" className="py-20">
      <div className="container mx-auto px-6">
        <motion.div
          className="max-w-5xl mx-auto text-center"
          initial={{ opacity: 0, y: 50 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        >
          <h2 className="text-5xl md:text-6xl font-bold mb-8 text-gradient">
            {content.heading}
          </h2>

          <p className="text-xl text-gray-300 mb-12">
            {content.subheading}
          </p>

          <div className="glass-effect rounded-3xl p-8 md:p-12 mb-12">
            {/* Email & Phone */}
            <div className="grid md:grid-cols-2 gap-6 mb-8">
              <motion.a
                href={`mailto:${content.email}`}
                className="flex items-center gap-4 p-4 rounded-2xl border border-white/10 hover:border-primary/50 group transition-colors"
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.97 }}
              >
                <span className="p-2 rounded-xl bg-primary/10 text-primary group-hover:bg-primary/20 transition-colors">
                  <EmailIcon />
                </span>
                <div className="text-left">
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-0.5">Email</p>
                  <p className="text-white group-hover:text-primary transition-colors text-sm font-medium">
                    {content.email}
                  </p>
                </div>
              </motion.a>

              {content.showPhone && (
                <motion.a
                  href={`tel:${content.phone.replace(/\s+/g, '')}`}
                  className="flex items-center gap-4 p-4 rounded-2xl border border-white/10 hover:border-primary/50 group transition-colors"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.97 }}
                >
                  <span className="p-2 rounded-xl bg-primary/10 text-primary group-hover:bg-primary/20 transition-colors">
                    <PhoneIcon />
                  </span>
                  <div className="text-left">
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-0.5">Phone</p>
                    <p className="text-white group-hover:text-primary transition-colors text-sm font-medium">
                      {content.phone}
                    </p>
                  </div>
                </motion.a>
              )}
            </div>

            {/* Social links */}
            <div className="flex flex-wrap justify-center gap-4">
              {content.links.map((link) => (
                <motion.a
                  key={link.label}
                  href={link.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-3 px-5 py-3 rounded-2xl border border-white/10 hover:border-primary/50 group transition-colors"
                  whileHover={{ y: -3, scale: 1.02 }}
                  whileTap={{ scale: 0.97 }}
                >
                  <span className="p-1.5 rounded-lg bg-primary/10 text-primary group-hover:bg-primary/20 transition-colors">
                    {LINK_ICONS[link.icon] ?? null}
                  </span>
                  <div className="text-left">
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-0.5">{link.label}</p>
                    <p className="text-white group-hover:text-primary transition-colors text-sm font-medium">
                      {link.handle}
                    </p>
                  </div>
                </motion.a>
              ))}
            </div>
          </div>

          <motion.a
            href={`mailto:${content.email}`}
            className="inline-block px-10 py-4 bg-gradient-to-r from-primary to-secondary rounded-full font-semibold text-lg text-gray-900"
            whileHover={{ scale: 1.04, boxShadow: '0 0 20px rgba(250, 204, 21, 0.45)' }}
            whileTap={{ scale: 0.96 }}
          >
            Send Me a Message
          </motion.a>
        </motion.div>
      </div>
    </section>
  )
}

export default Contact
